...
//
//edited: When a page-fault occurs on a COW page, allocate a new page with kalloc(), 
...
usertrap(void)
{
  ...
  if(r_scause() == 8){
    ...
    //the following is same as lazy allocaion lab
    ...
    if (r_scause() == 13 || r_scause() == 15)
    {
      ...
      if (handel_faults(p->pagetable, va) != 0)
        p->killed = 1;
    }
    else
    {
   ...
   //'finish' make sure to end killing the processes
  finish:
  if(p->killed)
    exit(-1);
...
int
handel_faults(pagetable_t pagetable, uint64 va)
{
  uint64 pa;
  pte_t *pte;
  uint flags;
